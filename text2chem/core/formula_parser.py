...
def __parse_parentheses(init_formula, init_factor, curr_dict):
    ...
    for m in re.finditer(re_in_parentheses, init_formula):
        ...
        # 2023.1.29 Na2/3Ni1/3Co(1/3-x)Mn1/3AlxO2 init_formula = init_formula.replace(m.group(0), "") --> init_formula = init_formula.replace(m.group(0), "") if unit_sym_dict else init_formula.replace(m.group(0), m.group(1))
