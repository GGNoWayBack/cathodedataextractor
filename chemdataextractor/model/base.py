    ...
    def merge_all(self, other, strict=True):
        ...
        if self._binding_compatible(other):
            if type(self) != type(other):
                ...
                for field_name, field in self.fields.items():
                    if (not self[field_name] and other.get(field_name, None) and not field.never_merge):
                        did_merge = True
                        self[field_name] = other[field_name]
                        self.merge_confidence(other, field_name)
        self._consolidate_binding()
        ...
        if type(other) == type(self):
            ...
            for field_name, field in self.fields.items():
                if isinstance(field, ModelType):
                    ...
                    if (not field.ignore_when_merging and self[field_name] is not None and len(self[field_name])
                            and other[field_name] is not None and len(other[field_name])
                            and self[field_name] != other[field_name]):
                        match = False
                        break
                else:
                    if (not field.ignore_when_merging and self[field_name] is not None
                            and other[field_name] is not None
                            and self[field_name] != other[field_name]):
                        match = False
                        break
...
def sort_merge_candidates(merge_candidates, adjust_by_confidence=True):
    ...
    if adjust_by_confidence:
        ...
        return sorted(merge_candidates, key=lambda x: x[0])
